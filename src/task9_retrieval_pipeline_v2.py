...
DEFAULT_TOP_K = 5
RERANK_METHOD = "rrf"  # "rrf" | "cross_encoder" | "mmr"
...
def retrieve(
    ...
    if dense_results or sparse_results:
        merged = rerank_rrf(
            [dense_results, sparse_results] if (dense_results and sparse_results) else [dense_results or sparse_results],
            top_k=top_k * 2,
            k=60
        )
    ...
    if use_reranking and merged and RERANK_METHOD == "rrf":
        # RRF already used for merging, so reranked = merged
